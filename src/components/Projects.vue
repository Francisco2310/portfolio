...
<template>
  ...
  <a class="p-2 bg-background-dark/80 backdrop-blur-md rounded-lg text-white hover:text-primary transition-colors shadow-lg" :href="`${link}`">
  ...
  <h3 class="text-slate-900 dark:text-white text-2xl font-bold group-hover:text-primary transition-colors">{{ nome }}</h3>
  </div>
  <p class="text-slate-500 dark:text-slate-400 mb-6 line-clamp-2">{{ descricao }}</p>
  <div class="flex flex-wrap items-center justify-between gap-4">
  <div class="flex gap-3">
  </div>
  </div>
  <a class="text-primary font-bold text-sm uppercase tracking-wider flex items-center gap-2 hover:translate-x-1 transition-transform" :href="`${github}`">
